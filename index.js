...
const colorScales = {
  ...
  secondary: {
    name: "secondary",
    prefix: "secondary",
    hVar: "--op-color-secondary-h",
    sVar: "--op-color-secondary-s",
    lVar: "--op-color-secondary-l",
  },
  tertiary: {
    name: "tertiary",
    prefix: "tertiary",
    hVar: "--op-color-tertiary-h",
    sVar: "--op-color-tertiary-s",
    lVar: "--op-color-tertiary-l",
  },
  neutral: {
    name: "Neutral",
    prefix: "neutral",
  ...
  colorSteps.forEach((step) => {
    ...
    if (onText && altText) {
      if (step === "original") {
        ...
        const stepName = step.replace(/\b\w/g, (l) => l.toUpperCase());
...
function updateAccessibilityIndicators(scalePrefix, step) {
  try {
    ...
    if (
      !scaleHSL ||
      scaleHSL.h === null ||
      scaleHSL.h === undefined ||
      scaleHSL.s === null ||
      scaleHSL.s === undefined ||
      isNaN(scaleHSL.h) ||
      isNaN(scaleHSL.s)
    ) {
      return;
    ...
    if (altIndicator) {
      altIndicator.textContent = `${altLevel} (${altContrastRatio.toFixed(
        2
      )}:1)`;
    ...
    console.warn(
      `Failed to update accessibility for ${scalePrefix}-${step}:`,
      error
    );
  }
}
...
function applyOverridesToCSS() {
  Object.keys(updatedLightnessValues).forEach((key) => {
    ...
    if (!override) return;
    const varName = `--op-color-${scalePrefix}-on-${step}${
      type === "alt" ? "-alt" : ""
    }`;
...
function fixAccessibility(scalePrefix, step, type) {
  try {
    ...
    let currentTextLightness =
      type === "alt"
        ? prefersDark
          ? defAltLightDark[step]
          : defAltLightLight[step]
        : prefersDark
        ? defOnLightDark[step]
        : defOnLightLight[step];
    ...
    const bgHex = bgColor;
    const currentTextHex = hslToHex(
      scaleHSL.h,
      scaleHSL.s,
      currentTextLightness
    );
    ...
    if (currentRatio >= targetRatio) {
      console.log(
        `Fixed accessibility for ${scalePrefix}-${step}-${type}: ${currentTextLightness}% lightness`
      );
      return;
    }
    ...
    for (let L = 0; L <= 100; L++) {
      ...
      candidates.push({ L, ratio: getContrastRatio(hex, bgHex) });
    }

    // Prefer library suggestion when available
    let libSuggestedLightness;
    try {
      if (window && typeof window.a11yContrastColor === "function") {
        const res = window.a11yContrastColor(bgHex, {
          algorithm: "wcag",
          level: "AA",
          size: "normal",
        });
        if (res && res.color) {
          ...
          if (rgb) {
            ...
            libSuggestedLightness = Math.round(((max + min) / 2) * 100);
          }
        }
      }
    } catch (_) {}
    ...
    let chosen = null;
    if (passing.length) {
      const selectionTargetLightness =
        typeof libSuggestedLightness === "number"
          ? libSuggestedLightness
          : currentTextLightness;
      chosen = passing.reduce((best, cur) => {
        const dCur = Math.abs(cur.L - selectionTargetLightness);
        if (!best) return cur;
        const dBest = Math.abs(best.L - selectionTargetLightness);
      ...
      const e100 = candidates[candidates.length - 1];
      chosen = e0 && e100 ? (e0.ratio >= e100.ratio ? e0 : e100) : e0 || e100;
    }

    const closestLightness = chosen ? chosen.L : currentTextLightness;
    ...
    if (prefersDark) {
      updatedLightnessValues[key] = {
        ...
        dark: closestLightness,
        updated: Date.now(),
      };
    } else {
      updatedLightnessValues[key] = {
        light: closestLightness,
    ...
    console.log(
      `Fixed accessibility for ${scalePrefix}-${step}-${type}: ${closestLightness}% lightness`
    );
  } catch (error) {
    console.warn(
      `Failed to fix accessibility for ${scalePrefix}-${step}-${type}:`,
      error
    );
  }
}
...
function getScaleHSL(scalePrefix) {
  ...
  const computedStyle = getComputedStyle(document.documentElement);
  const hValue = computedStyle
    .getPropertyValue(`--op-color-${scalePrefix}-h`)
    .trim();
  const sValue = computedStyle
    .getPropertyValue(`--op-color-${scalePrefix}-s`)
    .trim();
  const lValue = computedStyle
    .getPropertyValue(`--op-color-${scalePrefix}-l`)
    .trim();
...
const hslInputs = [
  ...
  {
    idH: "hsl-alerts-warning-h",
    idS: "hsl-alerts-warning-s",
    prefix: "alerts-warning",
  },
  {
    idH: "hsl-alerts-danger-h",
    idS: "hsl-alerts-danger-s",
    prefix: "alerts-danger",
  },
  {
    idH: "hsl-alerts-info-h",
    idS: "hsl-alerts-info-s",
    prefix: "alerts-info",
  },
  {
    idH: "hsl-alerts-notice-h",
    idS: "hsl-alerts-notice-s",
    prefix: "alerts-notice",
  },
];
...
function initHSLControls() {
  ...
  hslInputs.forEach(({ idH, idS, prefix }) => {
...
function copyVariables() {
  ...
  navigator.clipboard
    ...
    .then(() => {
      ...
      button.style.background = "var(--op-color-primary-base)";
...
function generateAndDownloadJSON() {
  ...
  button.style.background = "var(--op-color-primary)";
